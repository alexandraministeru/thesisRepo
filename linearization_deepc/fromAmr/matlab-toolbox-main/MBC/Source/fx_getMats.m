...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Modification by Amr %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% A problem was raised regarding the description of the UserProp AD inputs that they have Nul characters.
% Accordingly, a check is to be made if those Nul characters exist or not.
% If they do, then they will be replaced with the proper description.
idxAD = find(contains(matData.DescCntrlInpt,'AD'));
AD_UserProp = matData.DescCntrlInpt(idxAD);
LenDescCntrlInpt = length(AD_UserProp{1});
BldNum = 1;                     % A counter for the number of blades (Total BldNum = 3) 
NodeNum = 1;                    % A counter for the number of nodes per blade (Total NodeNum = 19)
DescOpt1 = 'AD User property on blade #, node #, -';
DescOpt2 = 'AD User property on blade #, node ##, -';

if LenDescCntrlInpt < length(DescOpt1) || LenDescCntrlInpt > length(DescOpt2)
    for ii = 1:length(AD_UserProp)         
        AD_UserProp{ii} = ['AD User Property on blade ', num2str(BldNum), ', node ', num2str(NodeNum), ', -'];
        NodeNum = NodeNum + 1;
...
matData.DescCntrlInpt(idxAD) = AD_UserProp;
