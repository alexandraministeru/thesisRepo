...
MBC.DescCntrlInpt = matData.DescCntrlInpt; % save this in the MBC type
MBC.DescOutput = matData.DescOutput; % save this in the MBC type
...
if isfield(MBC,'A')
    ...
    % Sometimes in the A matrix, NaN is found in the rows corresponding to
    % the hydrodynamic radiation states
    if anynan(MBC.A)
        [row,col,page] = ind2sub(size(MBC.A),find(isnan(MBC.A)));
        PageClean = setdiff((1:size(MBC.A,3)), unique(page));
        MBC.A(unique(row),unique(col),unique(page)) = MBC.A(unique(row),unique(col),PageClean(1)); 
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(MBC,'B')
    %%%%%%%%%%%%%%%%%%%%%%%% Modification by Amr %%%%%%%%%%%%%%%%%%%%%%%%%%
    % Motivation for the modification
    % Sometimes in the B matrix, NaN is found in the rows corresponding to
    % the hydrodynamic radiation states
    if anynan(MBC.B)
        [row,col,page] = ind2sub(size(MBC.B),find(isnan(MBC.B)));
        PageClean = setdiff((1:size(MBC.B,3)), unique(page));
        MBC.B(unique(row),unique(col),unique(page)) = MBC.B(unique(row),unique(col),PageClean(1)); 
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(MBC,'C')
    %%%%%%%%%%%%%%%%%%%%%%%% Modification by Amr %%%%%%%%%%%%%%%%%%%%%%%%%%
    % Motivation for the modification
    % Sometimes in the C matrix, NaN is found in the rows corresponding to
    % the hydrodynamic radiation states
    if anynan(MBC.C)
        [row,col,page] = ind2sub(size(MBC.C),find(isnan(MBC.C)));
        PageClean = setdiff((1:size(MBC.C,3)), unique(page));
        MBC.C(unique(row),unique(col),unique(page)) = MBC.C(unique(row),unique(col),PageClean(1)); 
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(MBC,'D')
    %%%%%%%%%%%%%%%%%%%%%%%% Modification by Amr %%%%%%%%%%%%%%%%%%%%%%%%%%
    % Motivation for the modification
    % Sometimes in the D matrix, NaN is found in the rows corresponding to
    % the hydrodynamic radiation states
    if anynan(MBC.D)
        [row,col,page] = ind2sub(size(MBC.D),find(isnan(MBC.D)));
        PageClean = setdiff((1:size(MBC.D,3)), unique(page));
        MBC.D(unique(row),unique(col),unique(page)) = MBC.D(unique(row),unique(col),PageClean(1)); 
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
