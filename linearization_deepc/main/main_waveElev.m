...
waveHeight = data(:,ismember(channels,'Wave1Elev'));
...
inputChannels = {'IfW Extended input: horizontal wind speed (steady/uniform wind), m/s',...     
    ...
    'HD Extended input: wave elevation at platform ref point, m'};
...
% Wave disturbance input: elevation
u_waveHeight = waveHeight(1:length(u_bladePitch));
...
% 
% % Save input to file
% save('inputData\peinput.mat','u_bladePitch','u_waveHeight','windData');

% Load input from file
% load('inputData\peinput.mat')

% % Can replace wind disturbance:
% u_waveHeight = zeros(size(u_bladePitch)); % no wind disturbance
% % or
% load('inputData\turbWind_16mps.mat') %turbulent wind obtained from a previous FAST simulation
% u_waveHeight = windData(1:size(u_bladePitch,1));
% u_waveHeight = u_waveHeight-16; % around linearization point

u = [u_windSpeed u_bladePitch u_waveHeight];
...
disturbMat = [u_windSpeed u_waveHeight];
...
% Waves
wH = waveHeight(size(u_waveHeight,1)+1:end); 
...
for k=1:kFinal
    ...
    % Wind preview
    if previewFlag == 1
        previewData = [v(k:k+f-1) wH(k:k+f-1)];
   ...
   % Update past data with most recent I/O data
    ...
    if previewFlag == 1
        data.wini = [data.wini(nDist+1:end); v(k); wH(k)];
    end
end
