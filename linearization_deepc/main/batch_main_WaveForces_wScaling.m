...
u_batch = zeros(kFinal,1);
y_batch = zeros(kFinal,1);
d_batch = zeros(kFinal,1);
...
for k=1:kFinal
    ...
    if mod(k-1,N) == 0 && k ~= 1
        data.Up = constructHankelMat(u_batch(k-N:k-1),i,p,Nbar);
        data.Yp = constructHankelMat(y_batch(k-N:k-1),i,p,Nbar);

        % Future data
        data.Uf = constructHankelMat(u_batch(k-N:k-1),i+p,f,Nbar);
        data.Yf = constructHankelMat(y_batch(k-N:k-1),i+p,f,Nbar);        

        if previewFlag == 1
            data.Wp = constructHankelMat(d_batch(k-N:k-1),i,p,Nbar); % past data
            data.Wf = constructHankelMat(d_batch(k-N:k-1),i+p,f,Nbar); % future data
        else
            data.Wp = [];
            data.Wf = [];
        end
...
if saveFlag == 1
    simType = 'wave_ptfmPitch_bounded';
    ...
    while isfile(['updateMeeting3\' fileName ext])
    ...
    % Save variables
    if scaledFlag == 1
        save(['updateMeeting3\' fileName ext],'inName','outName','tsim','kFinal','ref','Ts','controlParams', 'out', ...
